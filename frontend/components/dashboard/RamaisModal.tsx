...
export function RamaisModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200 }}
      ...
      <div style={{ background: '#fff', borderRadius: 10, width: 420, maxHeight: '80vh', display: 'flex', flexDirection: 'column', boxShadow: '0 10px 40px rgba(0,0,0,.18)' }}>
        <div style={{ padding: '18px 22px 14px', borderBottom: '1px solid #e5e7eb' }}>
          ...
          <input value={busca} onChange={e => setBusca(e.target.value)} placeholder="Buscar por nome, setor ou número..."
            style={{ width: '100%', padding: '8px 10px', borderRadius: 5, border: '1px solid #d1d5db', fontSize: 13, boxSizing: 'border-box' }} />
        </div>

        <div style={{ overflowY: 'auto', flex: 1 }}>
          {loading && <p style={{ padding: 20, textAlign: 'center', color: '#6b7280', fontSize: 13 }}>Carregando...</p>}
          ...
          {!loading && filtrados.map(r => (
            <div key={r.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center',
              ...
              <div style={{ minWidth: 0 }}>
                <div style={{ fontSize: 13, fontWeight: 600, color: '#374151' }}>{r.nome}</div>
                {r.setor && <div style={{ fontSize: 11.5, color: '#9ca3af' }}>{r.setor}</div>}
              </div>
