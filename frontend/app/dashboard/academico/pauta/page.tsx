  ...
  return (
    <span
      ...
      style={{
        ...
        color: valor === null || valor === undefined ? '#93c5fd' : (Number(valor) < 6 ? '#dc2626' : '#111827'),
  ...
  return (
    <div style={{ padding: '24px 28px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 2fr', gap: 12, marginBottom: 14, background: '#f9fafb', padding: 16, borderRadius: 8, border: '1px solid #e5e7eb' }}>
        ...
        <div>
          <label style={{ fontSize: 12, fontWeight: 600, color: '#374151', display: 'block', marginBottom: 4 }}>Curso</label>
        ...
        <div>
          <label style={{ fontSize: 12, fontWeight: 600, color: '#374151', display: 'block', marginBottom: 4 }}>Turma / Matéria</label>
      ...
      {!loading && !ofertaId && <p style={{ color: '#6b7280', fontSize: 13 }}>Selecione uma turma/matéria pra ver a pauta.</p>}
            ...
            <thead>
              <tr style={{ background: '#f9fafb' }}>
                {['Matrícula', 'Nome', 'Nº', 'AV1', 'AV2', 'AV3', 'AV4', 'AV5', '2°', 'Recup.', 'Média', 'Faltas'].map(h => (
                  <th key={h} style={{ padding: '8px 10px', fontWeight: 700, color: '#374151', textAlign: h === 'Nome' ? 'left' : 'center', borderBottom: '1px solid #e5e7eb', borderRight: '1px solid #f3f4f6', whiteSpace: 'nowrap' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {pauta.linhas.map((l, i) => (
                <tr key={l.matriculaDisciplinaId} style={{ borderBottom: '1px solid #f3f4f6', background: i % 2 ? '#fafafa' : '#fff' }}>
                  <td style={{ padding: '6px 10px', fontWeight: 600, borderRight: '1px solid #f3f4f6' }}>{l.aluno.ra}</td>
                  <td style={{ padding: '6px 10px', borderRight: '1px solid #f3f4f6' }}>{l.aluno.nome}</td>
                  <td style={{ padding: '6px 10px', textAlign: 'center', borderRight: '1px solid #f3f4f6' }}>{l.numero}</td>
                  {(['av1', 'av2', 'av3', 'av4', 'av5', 'segundaChamada', 'recuperacao'] as CampoNota[]).map(campo => (
                    <td key={campo} style={{ padding: '2px', textAlign: 'center', borderRight: '1px solid #f3f4f6' }}>
                      <CelaNota valor={l[campo]} salvando={salvandoId === l.matriculaDisciplinaId} onSave={v => salvarCampo(l.matriculaDisciplinaId, campo, v)} />
                    </td>
                  ))}
                  <td style={{ padding: '6px 10px', textAlign: 'center', fontWeight: 700, borderRight: '1px solid #f3f4f6' }}>{formatNota(l.media)}</td>
