    ...
    <div style={{ padding: '24px 28px', maxWidth: 720 }}>
      ...
      <div style={{ background: 'var(--white)', border: '1px solid var(--gray-200)', borderRadius: 8, padding: 20, marginBottom: 20 }}>
        <div style={{ display: 'flex', gap: 8, alignItems: 'flex-end', flexWrap: 'wrap' }}>
          <div>
            <label style={{ fontSize: 12, fontWeight: 600, color: 'var(--gray-700)', display: 'block', marginBottom: 4 }}>Tipo de documento</label>
        ...
        <p style={{ fontSize: 11, color: 'var(--gray-400)', margin: '10px 0 0' }}>PDF, DOC/DOCX, JPG, PNG ou WEBP — até 10MB.</p>
      ...
      <div style={{ background: 'var(--white)', border: '1px solid var(--gray-200)', borderRadius: 8, overflow: 'hidden' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
          ...
          <tbody>
            {documentos.length === 0 && (
              <tr><td colSpan={5} style={{ padding: 24, textAlign: 'center', color: 'var(--gray-400)' }}>Nenhum documento enviado ainda.</td></tr>
            )}
            {documentos.map(d => (
              <tr key={d.id} style={{ borderBottom: '1px solid var(--gray-100)' }}>
                ...
                <td style={{ padding: '10px 14px', color: 'var(--gray-500)', fontSize: 12 }}>{fmtBytes(d.tamanho)}</td>
                <td style={{ padding: '10px 14px', color: 'var(--gray-500)', fontSize: 12 }}>{new Date(d.criadoEm).toLocaleDateString('pt-BR')}</td>
                <td style={{ padding: '10px 14px' }}>
