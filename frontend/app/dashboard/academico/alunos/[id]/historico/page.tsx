...
const STATUS_STYLE: Record<string, { bg: string; color: string }> = {
  ...
  TRANCADO:      { bg: '#f3f4f6', color: '#374151' },
};
...
export default function HistoricoAlunoPage() {
  ...
  return (
    <div style={{ maxWidth: 900, margin: '0 auto' }}>
      ...
      {/* Cabeçalho do aluno */}
      <div style={{ background: '#fff', borderRadius: 8, border: '1px solid #e5e7eb', padding: 20, marginBottom: 16 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 12 }}>
          <div>
            <h1 style={{ margin: '0 0 4px', fontSize: 20, fontWeight: 700 }}>{aluno.nome}</h1>
            <p style={{ margin: 0, fontSize: 13, color: '#6b7280' }}>RA: {aluno.ra} · {aluno.situacaoVinculo}</p>
          </div>
          {/* CR em destaque */}
          <div style={{ textAlign: 'center', background: '#f0f9ff', borderRadius: 8, padding: '12px 20px', border: '1px solid #bae6fd' }}>
            ...
            <div style={{ fontSize: 10, color: '#6b7280', marginTop: 2 }}>escala 0–10</div>
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10, marginTop: 16 }}>
          {[
            { label: 'Total de disciplinas', value: totalDisciplinas, color: '#374151' },
      ...
      <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 16 }}>
        ...
        <select style={{ padding: '6px 10px', borderRadius: 5, border: '1px solid #d1d5db', fontSize: 13 }}
      ...
      {Object.keys(porPeriodo).sort().map(periodo => (
        <div key={periodo} style={{ marginBottom: 20 }}>
          <h3 style={{ margin: '0 0 8px', fontSize: 13, fontWeight: 700, color: '#374151', textTransform: 'uppercase', letterSpacing: 0.5 }}>
            {periodo}
          </h3>
              ...
              <thead>
                <tr style={{ background: '#f9fafb', borderBottom: '1px solid #e5e7eb' }}>
                  {['Código', 'Disciplina', 'Prof.', 'Média', 'Freq.', 'Faltas', 'Situação', 'Status', ''].map(h => (
                    <th key={h} style={{ padding: '8px 12px', textAlign: 'left', fontWeight: 600, color: '#374151', fontSize: 11 }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {porPeriodo[periodo].map((m, i) => {
                  ...
                  return (
                    <tr key={m.id} style={{ borderBottom: '1px solid #f3f4f6', background: i % 2 ? '#fafafa' : '#fff' }}>
                      ...
                      <td style={{ padding: '9px 12px', fontSize: 12, color: '#6b7280', maxWidth: 120, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                        {m.oferta.professor.nome.split(' ')[0]}
                      </td>
                      <td style={{ padding: '9px 12px', fontWeight: 700, color: res ? (Number(res.mediaFinal) >= 6 ? '#065f46' : '#e02424') : '#9ca3af' }}>
                        {res ? Number(res.mediaFinal).toFixed(1) : '—'}
                      </td>
                      <td style={{ padding: '9px 12px', color: res ? (Number(res.frequenciaPercentual) >= 75 ? '#065f46' : '#e02424') : '#9ca3af' }}>
                      ...
                      <td style={{ padding: '9px 12px' }}>
                        <button
                          style={{ padding: '3px 8px', borderRadius: 4, border: '1px solid #d1d5db', background: 'transparent', cursor: 'pointer', fontSize: 11, color: '#374151' }}
      ...
      {filtered.length === 0 && (
        <p style={{ color: '#9ca3af', fontSize: 14, textAlign: 'center', padding: 24 }}>
