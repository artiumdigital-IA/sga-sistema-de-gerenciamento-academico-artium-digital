  ...
  return (
    <div style={{ padding: '24px 28px' }}>
      <div style={{ marginBottom: 20 }}>
        ...
        <p style={{ margin: 0, fontSize: 12, color: '#6b7280' }}>Compor mensagem direcionada a um usuário do sistema, e consultar enviadas/recebidas.</p>
      </div>

      <div style={{ display: 'flex', gap: 8, marginBottom: 20, borderBottom: '1px solid #e5e7eb' }}>
        {(['compor', 'enviadas', 'recebidas'] as const).map(t => (
          <button key={t} onClick={() => setTab(t)} style={{ padding: '8px 16px', border: 'none', background: 'transparent', cursor: 'pointer', fontSize: 13, fontWeight: 600, borderBottom: tab === t ? '2px solid #7c3aed' : '2px solid transparent', color: tab === t ? '#7c3aed' : '#6b7280', textTransform: 'capitalize' }}>
      ...
      {tab === 'compor' && (
        <div style={{ background: '#f9fafb', border: '1px solid #e5e7eb', borderRadius: 8, padding: 18, maxWidth: 560 }}>
      ...
      {tab === 'enviadas' && (
        loading ? <p style={{ color: '#6b7280', fontSize: 13 }}>Carregando...</p> : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {enviadas.length === 0 && <p style={{ color: '#9ca3af', fontSize: 13 }}>Nenhuma mensagem enviada.</p>}
            {enviadas.map(m => (
              <div key={m.id} style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 8, padding: '12px 16px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                  <strong style={{ fontSize: 13 }}>{m.assunto}</strong>
                  <span style={{ fontSize: 11, color: '#9ca3af' }}>{new Date(m.criadoEm).toLocaleString('pt-BR')}</span>
                </div>
                <p style={{ margin: '0 0 6px', fontSize: 12, color: '#6b7280' }}>Para: {m.destinatario?.nome ?? m.destinatario?.email}</p>
      ...
      {tab === 'recebidas' && (
        loading ? <p style={{ color: '#6b7280', fontSize: 13 }}>Carregando...</p> : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {recebidas.length === 0 && <p style={{ color: '#9ca3af', fontSize: 13 }}>Nenhuma mensagem recebida.</p>}
            {recebidas.map(m => (
              <div key={m.id} style={{ background: m.lida ? '#fff' : '#eff6ff', border: '1px solid #e5e7eb', borderRadius: 8, padding: '12px 16px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                  <strong style={{ fontSize: 13 }}>{m.assunto}</strong>
                  <span style={{ fontSize: 11, color: '#9ca3af' }}>{new Date(m.criadoEm).toLocaleString('pt-BR')}</span>
                </div>
                <p style={{ margin: '0 0 6px', fontSize: 12, color: '#6b7280' }}>De: {m.remetente?.nome ?? m.remetente?.email}</p>
