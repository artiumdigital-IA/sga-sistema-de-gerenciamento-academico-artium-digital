...
export default function TiposProtocoloPage() {
  ...
  const carregar = useCallback(async () => {
    setLoading(true);
    try { setTipos(await apiFetch<TipoProtocolo[]>('/tipos-protocolo')); }
  ...
  async function salvar() {
    ...
    setSalvando(true);
    try {
      await apiFetch('/tipos-protocolo', { method: 'POST', body: JSON.stringify({ nome }) });
      setNome(''); carregar();
    } catch (e: any) { alert(e.message ?? 'Erro ao salvar'); }
    finally { setSalvando(false); }
  }

  async function alternarAtivo(t: TipoProtocolo) {
    try { await apiFetch(`/tipos-protocolo/${t.id}`, { method: 'PATCH', body: JSON.stringify({ ativo: !t.ativo }) }); carregar(); }
    catch (e: any) { alert(e.message ?? 'Erro'); }
  }

  async function remover(id: string) {
    if (!confirm('Remover este tipo de protocolo?')) return;
    try { await apiFetch(`/tipos-protocolo/${id}`, { method: 'DELETE' }); carregar(); }
  ...
  return (
    <div style={{ padding: '24px 28px' }}>
      <div style={{ marginBottom: 20 }}>
        <h1 style={{ margin: '0 0 4px', fontSize: 20, fontWeight: 700 }}>Tipos de Protocolo</h1>
        <p style={{ margin: 0, fontSize: 12, color: '#6b7280' }}>Catálogo de tipos usados no lançamento de protocolos.</p>
      </div>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 20, alignItems: 'end', background: '#f9fafb', padding: 16, borderRadius: 8, border: '1px solid #e5e7eb' }}>
        <div style={{ flex: 1 }}>
          ...
          <input style={INPUT} value={nome} placeholder="Ex: Solicitação de 2ª via" onChange={e => setNome(e.target.value)} />
        </div>
      ...
      {loading ? <p style={{ color: '#6b7280', fontSize: 13 }}>Carregando...</p> : (
        <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 8, overflow: 'hidden' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            ...
            <tbody>
              {tipos.length === 0 && <tr><td colSpan={3} style={{ padding: 24, textAlign: 'center', color: '#9ca3af' }}>Nenhum tipo cadastrado.</td></tr>}
              {tipos.map(t => (
                <tr key={t.id} style={{ borderBottom: '1px solid #f3f4f6' }}>
                  <td style={{ padding: '10px 14px' }}>{t.nome}</td>
                  <td style={{ padding: '10px 14px' }}>
                    <button onClick={() => alternarAtivo(t)} style={{ padding: '2px 8px', borderRadius: 999, fontSize: 11, fontWeight: 600, border: 'none', cursor: 'pointer', background: t.ativo ? '#d1fae5' : '#f3f4f6', color: t.ativo ? '#065f46' : '#6b7280' }}>
