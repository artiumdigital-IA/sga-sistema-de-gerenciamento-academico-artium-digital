  ...
  return (
    <div style={{ padding: '24px 28px' }}>
      <div style={{ marginBottom: 20 }}>
        ...
        <p style={{ margin: 0, fontSize: 12, color: 'var(--gray-500)' }}>Trilha de auditoria (LGPD) — quem alterou o quê, e quando. Acesso restrito a Admin.</p>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr auto', gap: 10, marginBottom: 16, background: 'var(--gray-50)', padding: 16, borderRadius: 8, border: '1px solid var(--gray-200)', alignItems: 'end' }}>
        <div>
      ...
      {!loading && data && (
        <>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
              ...
              <tbody>
                {data.registros.length === 0 && (
                  <tr><td colSpan={5} style={{ padding: 24, textAlign: 'center', color: 'var(--gray-400)' }}>Nenhum registro encontrado.</td></tr>
                )}
                {data.registros.map(r => {
                  const cor = ACAO_COLOR[r.acao] ?? { bg: 'var(--gray-100)', color: 'var(--gray-700)' };
                  return (
                    <tr key={r.id} style={{ borderBottom: '1px solid var(--gray-100)' }}>
                      <td style={{ padding: '10px 14px', fontSize: 12, color: 'var(--gray-500)' }}>{new Date(r.criadoEm).toLocaleString('pt-BR')}</td>
                      ...
                      <td style={{ padding: '10px 14px', fontFamily: 'monospace', fontSize: 11, color: 'var(--gray-400)' }}>{r.entidadeId ?? '—'}</td>
                    </tr>
                  );
                })}
              </tbody>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 12 }}>
            <span style={{ fontSize: 12, color: 'var(--gray-500)' }}>{data.total} registro{data.total !== 1 ? 's' : ''} — página {data.page} de {Math.max(data.totalPaginas, 1)}</span>
