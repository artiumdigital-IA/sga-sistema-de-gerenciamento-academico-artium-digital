  ...
  return (
    <div style={{ padding: '24px 28px' }}>
      <div style={{ marginBottom: 20 }}>
        ...
        <p style={{ margin: 0, fontSize: 12, color: '#6b7280' }}>Inadimplência, resumo por turma e resumo contábil por competência.</p>
      </div>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 20, borderBottom: '1px solid #e5e7eb' }}>
        {([
          ...
          <button key={v} onClick={() => setTab(v)} style={{ padding: '8px 16px', border: 'none', background: 'transparent', cursor: 'pointer', fontSize: 13, fontWeight: 600, borderBottom: tab === v ? '2px solid #1a56db' : '2px solid transparent', color: tab === v ? '#1a56db' : '#6b7280' }}>
            {l}
          </button>
        ))}
      </div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
              <thead>
                <tr style={{ background: '#f9fafb', borderBottom: '1px solid #e5e7eb' }}>
                  {['RA', 'Aluno', 'Período', 'Parcela', 'Valor', 'Vencimento', 'Dias em Atraso'].map(h => <th key={h} style={{ padding: '10px 14px', textAlign: 'left', fontWeight: 600, color: '#374151', fontSize: 12 }}>{h}</th>)}
                </tr>
              </thead>
              <tbody>
                {inadimplencia.linhas.length === 0 && <tr><td colSpan={7} style={{ padding: 24, textAlign: 'center', color: '#9ca3af' }}>Nenhuma parcela em atraso.</td></tr>}
      ...
      {!loading && tab === 'turma' && (
        <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 8, overflow: 'hidden' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            <thead>
              <tr style={{ background: '#f9fafb', borderBottom: '1px solid #e5e7eb' }}>
                {['Curso', 'Período', 'Contratos', 'Valor Total', 'Valor Pago', 'Valor Pendente'].map(h => <th key={h} style={{ padding: '10px 14px', textAlign: 'left', fontWeight: 600, color: '#374151', fontSize: 12 }}>{h}</th>)}
              </tr>
            </thead>
            <tbody>
              {turma.length === 0 && <tr><td colSpan={6} style={{ padding: 24, textAlign: 'center', color: '#9ca3af' }}>Nenhum contrato encontrado.</td></tr>}
              {turma.map((l, i) => (
      ...
      {!loading && tab === 'contabil' && (
        <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 8, overflow: 'hidden' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            <thead>
              <tr style={{ background: '#f9fafb', borderBottom: '1px solid #e5e7eb' }}>
                {['Competência', 'Curso', 'Qtd. Parcelas', 'Valor Recebido'].map(h => <th key={h} style={{ padding: '10px 14px', textAlign: 'left', fontWeight: 600, color: '#374151', fontSize: 12 }}>{h}</th>)}
              </tr>
            </thead>
            <tbody>
              {contabil.length === 0 && <tr><td colSpan={4} style={{ padding: 24, textAlign: 'center', color: '#9ca3af' }}>Nenhum recebimento encontrado.</td></tr>}
              {contabil.map((l, i) => (
                <tr key={i} style={{ borderBottom: '1px solid #f3f4f6' }}>
